  ...
  while (listentry) {
    if (listentry->data && *listentry->data) {
      if (result)
		...
		int fulladdress = ((strlen(listentry->data) > 0) && (listentry->data[strlen(listentry->data) - 1] == '>'));
		if (!fulladdress) str_append(&result, "<");
...
{
#if defined(NOCURLMAIL) && defined(_WIN32)
...
DLL_EXPORT_LIBQUICKMAIL quickmail quickmail_create (const char* from, const char* subject)
{
  ...
  mailobj->localname = NULL;
  mailobj->charset = NULL;
...
{
  free(mailobj->localname);
  free(mailobj->from);
  free(mailobj->charset);
  free(mailobj->mail_id);
  ...
  while (mailobj->buflen == 0) {
    if (mailobj->buflen == 0 && mailobj->current == MAILPART_HEADER) {
      ...
      if (mailobj->from && *mailobj->from) {
		  str_append(p, "From: ");
		  int fulladdress = ((strlen(mailobj->from) > 0) && (mailobj->from[strlen(mailobj->from) - 1] == '>'));
		  if (!fulladdress)  str_append(p, "<");
        str_append(p, mailobj->from);

		  if (!fulladdress)  str_append(p, ">" NEWLINE);
		  else str_append(p, NEWLINE);
      }
      ...
      if (mailobj->attachmentlist) {
        mailobj->mime_boundary_part = randomize_zeros(strdup("=PART=SEPARATOR=_0000_0000_0000_0000_0000_0000_="));
		  str_append(p, "Content-Type: multipart/mixed");
		  if (quickmail_get_charset(mailobj))
			{
				if (strlen(quickmail_get_charset(mailobj)) > 0)
				{
					str_append(&mailobj->buf, "; charset=");
					str_append(&mailobj->buf, quickmail_get_charset(mailobj));
				}
			}
		  str_append(p, "; boundary=\"");
...
DLL_EXPORT_LIBQUICKMAIL const char* quickmail_send (quickmail mailobj, const char* smtpserver, unsigned int smtpport, const char* username, const char* password)
{
  ...
  if (quickmail_get_localname)
  {
		strncpy(local_hostname, mailobj->localname, sizeof(local_hostname) -1);
  }
  else
  {
  ...
  if ((sock = socket_open(smtpserver, smtpport, &errmsg)) != INVALID_SOCKET) {
    ...
    if ((statuscode = socket_smtp_command(sock, mailobj->debuglog, NULL)) >= 400) {
      ...
      do {
        ...
        //send end of data
		  char *retmsg = NULL;
		  if ((statuscode = socket_smtp_command2(sock, &retmsg, "\r\n.")) >= 400) {
          errmsg = "SMTP error after sending message data";
        }
		  quickmail_set_mail_id(mailobj, retmsg);
		  free(retmsg);
...
void quickmail_set_localname(quickmail mailobj, const char *localname)
{
	free(mailobj->localname);
	mailobj->localname = strdup(localname);
}

const char *quickmail_get_localname(quickmail mailobj)
{
	return mailobj->localname;
}

void quickmail_set_charset(quickmail mailobj, const char *charset)
{
	free(mailobj->charset);
	mailobj->charset = strdup(charset);
}

const char *quickmail_get_charset(quickmail mailobj)
{
	return mailobj->charset;
}

const char *quickmail_set_mail_id(quickmail mailobj, const char *message)
{
	free(mailobj->mail_id);
	mailobj->mail_id = strdup(message);
}

const char *quickmail_get_mail_id(quickmail mailobj)
{
