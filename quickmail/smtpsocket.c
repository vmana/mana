...
SOCKET socket_open (const char* smtpserver, unsigned int smtpport, char** errmsg)
{
  ...
  struct linger linger_option; //linger 2 seconds when disconnecting
  linger_option.l_linger = -1;
  linger_option.l_onoff = 2;
...
int socket_smtp_command (SOCKET sock, FILE* debuglog, const char* tpl, ...)
{
  char* message;
  int statuscode;
  ...
  if (tpl) {
    ...
    va_copy(aq, ap);
	 cmdlen = vsnprintf(NULL, 0, tpl, aq);
    va_end(aq);
...
int socket_smtp_command2 (SOCKET sock, char **debuglog, const char* tpl, ...)
{
  char* message;
  int statuscode;
  //send command (if one is supplied)
